...
IMAGE_PATH= 'map.png' ## change it to the image that needs to be feed.
show_animation = True
pause_time = 0.001
...
def generate_obstacle_map(image_path: str) -> tuple:

    """
    Process an image to create a corresponding obstacle grid map.

    Args:
        image_path (str): Path to the input image file.

    Returns:
        tuple: A tuple containing two lists, obstacle x-coordinates and obstacle y-coordinates.
    """
    obstacle_x_coords = []
    obstacle_y_coords = []

    img = cv.imread(image_path)
    assert img is not None, "File could not be read. Please check using os.path.exists()"

    if len(img.shape) == 3:   # If the input is an RGB image, convert it to grayscale.
        img = cv.cvtColor(img, cv.COLOR_BGR2GRAY)

    for i in range(len(img)):
        for j in range(len(img[i])):
            if img[i][j] == 0:
                obstacle_x_coords.append(j)
                obstacle_y_coords.append(99 - i)

    return obstacle_x_coords, obstacle_y_coords
...
def main():
    ...
    ox,oy=generate_obstacle_map(IMAGE_PATH)
